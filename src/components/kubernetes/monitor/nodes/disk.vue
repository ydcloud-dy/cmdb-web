  ...
  if (MetricData.value.fsUsage) {
    if (MetricData.value.fsUsage.length > 0) {
      MetricData.value.fsUsage.forEach(function(item) {
        ...
        fsUsageValue.value.push(bytesToUnits(parseInt(item[1])))
      })
    }
  }

  if (MetricData.value.fsSize) {
    if (MetricData.value.fsSize.length > 0) {
      MetricData.value.fsSize.forEach(function(item) {
        MetricTime.value.push(formatTimeLabels(item[0]))
        fsSizeValue.value.push(bytesToUnits(parseInt(item[1])))
      })
    }
  }
