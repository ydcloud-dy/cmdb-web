...
const ClusterRoles = async() => {
  const res = await getClusterRoles({ cluster_id: props.form.id, role_type: 'cluster' })
  if (res.code === 0) {
...
const ClusterApiGroups = async() => {
  const res = await getClusterApiGroups({ cluster_id: cluster_id.value, api_type: 'cluster' })
  if (res.code === 0) {
