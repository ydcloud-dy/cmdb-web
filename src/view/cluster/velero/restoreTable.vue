<template>
  <div>
    <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
      ...
      <el-table-column align="left" label="备份名称" prop="backupName">
        <template #default="scope">
          <span class="operate-span-primary">{{ scope.row.spec.backupName }}</span>
        </template>
      </el-table-column>
      <el-table-column
          label="恢复状态"
      ...
      >
        <template #default="scope">
          <span>{{ scope.row.status.warnings }}</span>
        </template>
      </el-table-column>
      <el-table-column
          label="恢复错误数量"
...
const emit = defineEmits(['update', 'delete', 'search', 'reduction', 'yaml_update'])
defineProps({
  tableData: {
    default: function() {
      return []
...
const handleDetail = (value) => {
  emit('veleroRestoreDetail', value)
}
