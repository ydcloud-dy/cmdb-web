    ...
    <el-row :gutter="20">
      ...
      <el-col :span="6" :offset="3">
        <el-form-item label="名称" prop="name" class="required">
          <el-input v-model="formData.name" type="text" placeholder="请输入还原任务名称" clearable></el-input>
        </el-form-item>
...
const includedNamespacesOptions = [
  { "label": "还原所有命名空间", "value": 1 ,"disabled":true},
  { "label": "还原指定命名空间", "value": 2 }
]

const includedResourcesOptions = [
  { "label": "还原所有资源类型", "value": 1 },
  { "label": "还原指定资源类型", "value": 2 }
]
...
const formData = reactive({
  ...
  backupName: props.form?.spec?.backupName || ""
});
...
const createTasks = () => {
  ...
  console.log(formData.backupName,"00000000")
  // 验证表单
  vForm.value.validate((valid) => {
    if (valid) {
