<template>
  <div>
    <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
      ...
      <el-table-column label="调度" min-width="40" prop="spec.schedule" />
      <el-table-column
        label="最后的调度时间"
        min-width="70"
      ...
      >
        <template #default="scope">
          {{ formatTimeToStr(scope.row.status.lastScheduleTime) }}
        </template>
      </el-table-column>
      <el-table-column label="暂停" min-width="40" prop="spec.suspend">
        <template #default="scope">
          {{ scope.row.spec.suspend }}
        </template>
      </el-table-column>
