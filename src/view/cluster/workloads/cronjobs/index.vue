  ...
  <div>
    ...
    <div v-if="dialogAddVisible">
      <el-dialog v-model="dialogAddVisible" width="70%" :title="title">
        <template #default>
          <div class="dialog-footer">
            <CreateBlock :form="form" :read-only="readOnly" :cluster="searchInfo.cluster_id" :operation="type" :type="'cronjobs'" @close="closeDialog" @enter="enterDialog" />
          </div>
        </template>
      </el-dialog>
    </div>
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await CronjobGet({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    cronjobName: row.metadata.name})
  if (res.data.items) {
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      console.log("创建cronjob======")
      res = await CronjobCreate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        content: row
      })
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('cronjobs', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
...
const openAddDialog = () => {
  ...
  form.value = getK8sObject('cronjobsinit', searchInfo.value.namespace, '')
  dialogAddVisible.value = true
}
