  ...
  <div>
    ...
    <div v-if="dialogAddVisible">
      <el-dialog v-model="dialogAddVisible" width="70%" :title="title">
        <template #default>
          <div class="dialog-footer">
            <CreateBlock :form="form" :read-only="readOnly" :cluster="searchInfo.cluster_id" :operation="type" :type="'statefulsets'" @close="closeDialog" @enter="enterDialog" />
          </div>
        </template>
      </el-dialog>
    </div>
...
import {
  ...
  StatefulSetsCreate,
  StatefulSetsDelete,
} from '@/api/kubernetes/statefulsets'
...
const getTableData = async() => {
  const table = await StatefulSetsList({cluster_id:searchInfo.value.cluster_id,
    page:page.value, pageSize:pageSize.value, namespace:searchInfo.value.namespace,keyword: searchInfo.value.keywords})
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  ...
  title.value = '编辑'
  readOnly.value = true
  const res = await StatefulSetsGet({cluster_id:searchInfo.value.cluster_id,namespace: row.metadata.namespace,statefulsetName :row.metadata.name})
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await StatefulSetsDelete({cluster_id:searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,statefulsetName: row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await StatefulSetsCreate({cluster_id:searchInfo.value.cluster_id,namespace: row.metadata.namespace,content: row})
      break
    case 'update':
      res = await StatefulSetsUpdate({cluster_id:searchInfo.value.cluster_id,
        namespace: row.metadata.namespace, statefulsetName:row.metadata.name, content:row})
      break
    default:
      res = await StatefulSetsCreate({cluster_id:searchInfo.value.cluster_id, content:row})
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('statefulsets', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
...
const openAddDialog = () => {
  ...
  form.value = getK8sObject('statefulsetsinit', searchInfo.value.namespace, '')
  dialogAddVisible.value = true
}
