  ...
  <div>
    ...
    <div v-if="dialogAddVisible">
      <el-dialog v-model="dialogAddVisible" width="70%" :title="title">
        <template #default>
          <div class="dialog-footer">
            <CreateBlock :form="form" :read-only="readOnly" :cluster="searchInfo.cluster_id" :operation="type" :type="'jobs'" @close="closeDialog" @enter="enterDialog" />
          </div>
        </template>
      </el-dialog>
    </div>
...
const handleDelete = async(row) => {
  ...
  if (res.code === 0) {
    ...
    console.log("================================")
    console.log("nimabideaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('jobs', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
...
const openAddDialog = () => {
  ...
  form.value = getK8sObject('jobsinit', searchInfo.value.namespace, '')
  dialogAddVisible.value = true
}
