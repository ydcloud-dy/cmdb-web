...
<script setup>
import {
  PersistentVolumeClaimsCreate,
  PersistentVolumeClaimsDelete,
  PersistentVolumeClaimsGet,
  PersistentVolumeClaimsList, PersistentVolumeClaimsUpdate,
} from '@/api/kubernetes/persistentvolumeclaims'
...
const getTableData = async() => {
  const table = await PersistentVolumeClaimsList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await PersistentVolumeClaimsGet({
    cluster_id: searchInfo.value.cluster_id,
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await PersistentVolumeClaimsDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    pvcName: row.metadata.name})
  if (res.data.items) {
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await PersistentVolumeClaimsCreate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        content: row})
      break
    case 'update':
      res = await PersistentVolumeClaimsUpdate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        pvcName: row.metadata.name,
        content: row
...
const openDialog = () => {
  ...
  title.value = '创建'
  readOnly.value = false
  form.value = getK8sObject('persistentvolumeclaims', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
