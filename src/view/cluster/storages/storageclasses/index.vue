...
import {
  ...
  StorageClassDelete,
  StorageClassGet,
  StorageClassList, StorageClassUpdate,
} from '@/api/kubernetes/storageclasses'
...
const getTableData = async() => {
  const table = await StorageClassList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await StorageClassGet({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    storageClassName: row.metadata.name
  })
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await StorageClassDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    storageClassName: row.metadata.name
  })
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      ...
      break
    case 'update':
      res = await StorageClassUpdate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        storageName: row.metadata.name,
        content: row
      })
      break
    default:
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('storageclasses', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
...
const handleDetail = async(row) => {
  const res = await StorageClassGet({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    storageNameClass: row.metadata.name
  })
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
