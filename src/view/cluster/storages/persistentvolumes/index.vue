...
<script setup>
import {
  PersistentVolumesCreate,
  PersistentVolumesDelete,
  PersistentVolumesGet,
  PersistentVolumesList, PersistentVolumesUpdate,
} from '@/api/kubernetes/persistentvolumes'
...
const getTableData = async() => {
  const table = await PersistentVolumesList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await PersistentVolumesGet({
    cluster_id: searchInfo.value.cluster_id,
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await PersistentVolumesDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    pvName: row.metadata.name})
  if (res.data.items) {
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await PersistentVolumesCreate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        content: row
      })
      break
    case 'update':
      res = await PersistentVolumesUpdate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        pvName: row.metadata.name,
        content: row
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('persistentvolumes', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
