...
import {
  ...
  RoleBindingsDelete,
  RoleBindingsGet,
  RoleBindingsList, RoleBindingsUpdate,
} from '@/api/kubernetes/rolebindings'
...
const getTableData = async() => {
  const table = await RoleBindingsList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await RoleBindingsGet({
    cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, roleBindingName:row.metadata.name})
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await RoleBindingsDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    roleBindingName:row.metadata.name})
  if (res.data.items) {
...
const openDialog = () => {
  ...
  form.value = getK8sObject('rolebindings', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
