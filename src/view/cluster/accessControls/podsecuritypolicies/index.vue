...
<script setup>
import {
  PodSecurityPoliciesCreate,
  PodSecurityPoliciesDelete,
  PodSecurityPoliciesGet,
  PodSecurityPoliciesList, PodSecurityPoliciesUpdate,
} from '@/api/kubernetes/podsecuritypolicies'
...
const getTableData = async() => {
  const table = await PodSecurityPoliciesList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords, '')
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await PodSecurityPoliciesGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await PodSecurityPoliciesDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.data.items) {
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await PodSecurityPoliciesCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await PodSecurityPoliciesUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await PodSecurityPoliciesCreate(searchInfo.value.cluster_id, row)
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('podsecuritypolicies', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
