...
<script setup>
import {
  ClusterRoleBindingsCreate,
  ClusterRoleBindingsDelete,
  ClusterRoleBindingsGet,
  ClusterRoleBindingsList, ClusterRoleBindingsUpdate,
} from '@/api/kubernetes/clusterrolebindings'
...
const getTableData = async() => {
  const table = await ClusterRoleBindingsList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await ClusterRoleBindingsGet({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    clusterRoleBindingName: row.metadata.name})
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await ClusterRoleBindingsDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    clusterRoleBindingName: row.metadata.name
  })
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await ClusterRoleBindingsCreate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace, content: row})
      break
    case 'update':
      res = await ClusterRoleBindingsUpdate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        clusterRoleBindingName: row.metadata.name,
        content: row})
...
const openDialog = () => {
  ...
  title.value = '创建'
  readOnly.value = false
  form.value = getK8sObject('clusterrolebindings', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
