...
import {
  ...
  ClusterRolesDelete,
  ClusterRolesGet,
  ClusterRolesList, ClusterRolesUpdate,
} from '@/api/kubernetes/clusterroles'
...
const getTableData = async() => {
  const table = await ClusterRolesList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await ClusterRolesGet({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    clusterRoleName: row.metadata.name })
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await ClusterRolesDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    clusterRoleName: row.metadata.name})
  if (res.data.items) {
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await ClusterRolesCreate({
        cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, content:row})
      break
    case 'update':
      res = await ClusterRolesUpdate({ cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace,
        clusterRoleName:row.metadata.name, content:row})
      break
    default:
      res = await ClusterRolesCreate({cluster_id: searchInfo.value.cluster_id, content: row})
      break
  }
...
const openDialog = () => {
  ...
  form.value = getK8sObject('clusterroles', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}

// 详情
const handleDetail = async(row) => {
  const res = await ClusterRolesGet({cluster_id: searchInfo.value.cluster_id,namespace: row.metadata.namespace,clusterRoleName: row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
