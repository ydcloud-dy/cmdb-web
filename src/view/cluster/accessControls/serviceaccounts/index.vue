...
import {
  ...
  ServiceAccountsGet,
  ServiceAccountsList, ServiceAccountsUpdate,
} from '@/api/kubernetes/serviceaccounts'
...
const getTableData = async() => {
  const table = await ServiceAccountsList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await ServiceAccountsGet({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    serviceAccountName: row.metadata.name
  })
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await ServiceAccountsDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    serviceAccountName:row.metadata.name
})
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await ServiceAccountsCreate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        content:row
  })
      break
    case 'update':
      res = await ServiceAccountsUpdate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        serviceAccountName: row.metadata.name,
        content: row
      ...
      break
    default:
      res = await ServiceAccountsCreate({cluster_id: searchInfo.value.cluster_id,content: row})
      break
  }
...
const openDialog = () => {
  ...
  title.value = '创建'
  readOnly.value = false
  form.value = getK8sObject('serviceaccounts', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
