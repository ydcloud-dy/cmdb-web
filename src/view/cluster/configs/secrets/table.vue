    ...
    <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
      ...
      <el-table-column label="操作">
        <template #default="scope">
          ...
          <el-button v-if="SecretType(scope.row.type)" size="small" type="primary" link icon="edit" @click="handleUpdate(scope.row)">编辑</el-button>
          <el-button v-if="!SecretType(scope.row.type)" disabled size="small" type="primary" link icon="edit" @click="handleUpdate(scope.row)">编辑</el-button>
...
// 类型判断
const secretstypes = ref(['Opaque', 'kubernetes.io/dockerconfigjson', 'kubernetes.io/tls'])
const SecretType = (type) => {
  if (secretstypes.value.indexOf(type) > -1) {
    return true
  } else {
    return false
  }
