...
<script setup>
import {
  ResourceQuotasCreate,
  ResourceQuotasDelete,
  ResourceQuotasGet,
  ResourceQuotasList, ResourceQuotasUpdate,
} from '@/api/kubernetes/resourcequotas'
...
const getTableData = async() => {
  const table = await ResourceQuotasList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await ResourceQuotasGet({
    cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, resourcequotaName: row.metadata.name
  })
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await ResourceQuotasDelete({cluster_id: searchInfo.value.cluster_id,namespace: row.metadata.namespace, resourcequotaName:row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await ResourceQuotasCreate({cluster_id: searchInfo.value.cluster_id, namespace:row.metadata.namespace, content:row})
      break
    case 'update':
      res = await ResourceQuotasUpdate({cluster_id: searchInfo.value.cluster_id, namespace:row.metadata.namespace, resourcequotaName:row.metadata.name, content:row})
      break
    default:
      res = await ResourceQuotasCreate({cluster_id: searchInfo.value.cluster_id, content:row})
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('resourcequotas', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
...
const handleDetail = async(row) => {
  const res = await ResourceQuotasGet({cluster_id: searchInfo.value.cluster_id,namespace: row.metadata.namespace, resourcequotaName:row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
