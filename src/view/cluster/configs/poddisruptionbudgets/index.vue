...
<script setup>
import {
  PodDisruptionbudgetsCreate,
  PodDisruptionbudgetsDelete,
  PodDisruptionbudgetsGet,
  PodDisruptionbudgetsList, PodDisruptionbudgetsUpdate,
} from '@/api/kubernetes/poddisruptionbudgets'
...
const getTableData = async() => {
  const table = await PodDisruptionbudgetsList({
    cluster_id: searchInfo.value.cluster_id,
    page: page.value,
    pageSize: pageSize.value,
    namespce: searchInfo.value.namespace,
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await PodDisruptionbudgetsGet({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace,poddisruptionbudgetName: row.metadata.name})
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await PodDisruptionbudgetsDelete({cluster_id: searchInfo.value.cluster_id,namespace: row.metadata.namespace, poddisruptionbudgetName:row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await PodDisruptionbudgetsCreate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        content: row})
      break
    case 'update':
      res = await PodDisruptionbudgetsUpdate({
        cluster_id: searchInfo.value.cluster_id,
        namespace: row.metadata.namespace,
        poddisruptionbudgetName: row.metadata.name,
        content: row})
      break
    default:
      res = await PodDisruptionbudgetsCreate({
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('poddisruptionbudgets', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
...
const handleDetail = async(row) => {
  const res = await PodDisruptionbudgetsGet({
    cluster_id: searchInfo.value.cluster_id,namespace: row.metadata.namespace, poddisruptionbudgetName: row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
