...
<script setup>
import {
  HorizontalPodautoscalersCreate,
  HorizontalPodautoscalersDelete,
  HorizontalPodautoscalersGet,
  HorizontalPodautoscalersList, HorizontalPodautoscalersUpdate,
} from '@/api/kubernetes/horizontalpodautoscalers'
...
const getTableData = async() => {
  const table = await HorizontalPodautoscalersList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await HorizontalPodautoscalersGet({cluster_id: searchInfo.value.cluster_id, namespace:row.metadata.namespace, HorizontalPodName: row.metadata.name})
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await HorizontalPodautoscalersDelete({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    HorizontalPodName: row.metadata.name
  })
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await HorizontalPodautoscalersCreate({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, content: row})
      break
    case 'update':
      res = await HorizontalPodautoscalersUpdate({cluster_id :searchInfo.value.cluster_id,namespace: row.metadata.namespace, HorizontalPodName: row.metadata.name,content: row})
      break
    default:
      res = await HorizontalPodautoscalersCreate({cluster_id: searchInfo.value.cluster_id, content: row})
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('horizontalpodautoscalers', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
