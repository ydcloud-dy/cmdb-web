  ...
  <div>
    <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
      ...
      <el-table-column label="Workload" prop="spec.scaleTargetRef.kind">
        <template #default="scope">
          {{ scope.row.spec.scaleTargetRef.kind }}/{{ scope.row.spec.scaleTargetRef.name }}
        </template>
      </el-table-column>
      <el-table-column label="Min Replicas" prop="spec.scaleTargetRef.minReplicas ">
        <template #default="scope">
          {{ scope.row.spec.minReplicas }}
        </template>
      </el-table-column>
      <el-table-column label="Max Replicas" prop="spec.scaleTargetRef.maxReplicas">
        <template #default="scope">
          {{ scope.row.spec.maxReplicas }}
        </template>
      </el-table-column>
      <el-table-column label="Current Replicas" prop="status.currentReplicas">
        <template #default="scope">
          {{ scope.row.status.currentReplicas }}
        </template>
      </el-table-column>
      <el-table-column label="状态" prop="metadata.status">
        <template #default="scope">
          <el-tag
            v-if="scope.row.status.currentReplicas === scope.row.status.desiredReplicas"
            type="success"
            round
          >
            Active
