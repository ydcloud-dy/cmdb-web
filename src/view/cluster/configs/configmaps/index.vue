      ...
      >
        <template #default>
          <div class="dialog-footer">
            <FormBlock :cluster="searchInfo.cluster_id" :form="form" :readonly="readOnly" @close="closeDialog" @enter="enterDialog" />
          </div>
        </template>
      </el-dialog>
    </div>
...
import {
  ...
  ConfigmapsList,
  ConfigmapsUpdate,
} from '@/api/kubernetes/configmaps'
...
const getTableData = async() => {
  const table = await ConfigmapsList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await ConfigmapsGet({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, configmapName: row.metadata.name})
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await ConfigmapsDelete({cluster_id: searchInfo.value.cluster_id,namespace: row.metadata.namespace,configmapName: row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await ConfigmapsCreate({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, content:row})
      break
    case 'update':
      res = await ConfigmapsUpdate({cluster_id:searchInfo.value.cluster_id, namespace: row.metadata.namespace, configmapName: row.metadata.name,content: row})
      break
    default:
      res = await ConfigmapsCreate({cluster_id: searchInfo.value.cluster_id, content:row})
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('configmaps', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
...
const openAddDialog = async() => {
  ...
  title.value = '创建'
  readOnly.value = false
  form.value = getK8sObject('configmapsinit', searchInfo.value.namespace)
  dialogAddVisible.value = true
}

// 详情
const handleDetail = async(row) => {
  const res = await ConfigmapsGet({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, configmaoName: row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
