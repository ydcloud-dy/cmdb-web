  ...
  <div>
    <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
      ...
      <el-table-column label="Endpoints" prop="subsets" min-width="200px">
        <template #default="scope">
          <div v-for="(subset,index) in scope.row.subsets" :key="index">
            <div v-for="(address,index) in subset.addresses" :key="index" style="display:inline-block">
              <el-tag v-for="(port,index) in subset.ports" :key="index" type="info">{{
                address.ip
              }}:{{ port.port }}
              </el-tag>
            </div>
