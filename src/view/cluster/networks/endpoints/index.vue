...
const getTableData = async() => {
  const table = await EndpointsList({
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await EndpointsGet({
    cluster_id: searchInfo.value.cluster_id,
    namespace: row.metadata.namespace,
    endpointName: row.metadata.name})
  if (res.data.items) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await EndpointsDelete({cluster_id: searchInfo.value.cluster_id, namespace:row.metadata.namespace, endpointName:row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await EndpointsCreate({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace, content:row})
      break
    case 'update':
      res = await EndpointsUpdate({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace,
        endpointName:row.metadata.name, content: row})
      break
    default:
      res = await EndpointsCreate({cluster_id: searchInfo.value.cluster_id, content:row})
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('endpoints', searchInfo.value.namespace, '')
  readOnly.value = false
  dialogYamlVisible.value = true
}
