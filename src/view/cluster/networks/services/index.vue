...
const handleDelete = async(row) => {
  row.visible = false
  const res = await ServicesDelete({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace,serviceName: row.metadata.name})
  if (res.data.items) {
    if (res.data.items.code) {
      ElMessage({
        type: 'error',
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await ServicesCreate({cluster_id: searchInfo.value.cluster_id, namespace:row.metadata.namespace, content:row})
      break
    case 'update':
      res = await ServicesUpdate({cluster_id: searchInfo.value.cluster_id, namespace: row.metadata.namespace,
        serviceName:row.metadata.name, content: row})
      break
    default:
      res = await ServicesCreate({cluster_id: searchInfo.value.cluster_id, content:row})
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('services', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
...
const openAddDialog = () => {
  ...
  form.value = getK8sObject('servicesinit', searchInfo.value.namespace, '')
  dialogAddVisible.value = true
}
