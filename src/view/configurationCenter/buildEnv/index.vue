...
import { createBuildEnv,updateBuildEnv,deleteBuildEnvByIds,deleteBuildEnv,getBuildEnvList,describeBuildEnv} from "@/api/configurationCenter/buildEnv";
...
const getTableData = async() => {
  const table = await getBuildEnvList({ page: page.value, pageSize: pageSize.value, keyword: searchInfo.value.name })
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  const res = await describeBuildEnv(row.ID);
  type.value = 'update';
  title.value = '更新';
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await deleteBuildEnv(  row.ID )
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const onDelete = async() => {
  ...
  const res = await deleteBuildEnvByIds({ ids:ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: res.msg
...
const enterDialog = async (value) => {
  ...
  switch (type.value) {
    case "create":
      res = await createBuildEnv(value);
      break;
    case "update":
      res = await updateBuildEnv(value);
      break;
    default:
      res = await createBuildEnv(value);
      break;
