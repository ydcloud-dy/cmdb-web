...
import { virtualMachineList, syncMachine, virtualMachineTree } from '@/api/cloudCmdb/cloud_virtual_machine'
...
const searchFields = [{ 'id': 1, 'field': 'name', 'name': '实例名称' }, { 'id': 2, 'field': 'instance_id', 'name': '实例ID' }, { 'id': 8, 'field': 'os', 'name': '操作系统' }, { 'id': 9, 'field': 'os_type', 'name': '系统类型' }, { 'id': 10, 'field': 'private_addr', 'name': '私网地址' }, { 'id': 11, 'field': 'public_addr', 'name': '公网地址' }, { 'id': 12, 'field': 'region', 'name': '区域ID' }, { 'id': 13, 'field': 'region_name', 'name': '区域名称' }, { 'id': 14, 'field': 'status', 'name': '状态' }]

const GetvirtualMachineTree = async() => {
  const res = await virtualMachineTree()
  if (res.code === 0) {
...
const onSync = async() => {
  ...
  const res = await syncMachine({ 'id': cloud_id.value })
  if (res.code === 0) {
    ElMessage({
...
const getTableData = async() => {
  const table = await virtualMachineList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
